"""
=============================================================================
پروژه: سامانه جامع آموزشی، آزمون و پشتیبانی آکادمی (Telegram Academy Bot)
معماری: Webhook مبتنی بر aiohttp و پایگاه‌داده SQLite ناهمگام (aiosqlite)
نقش‌های کاربری: فراگیر (Student)، پشتیبان (Supporter)، استاد (Teacher)
=============================================================================
"""
...
# =============================================================================
# لاگر استاندارد (Structured Logging)
# =============================================================================
logging.basicConfig(
    level=logging.INFO,
...
WEB_APP_URL = os.getenv("WEB_APP_URL", "https://your-username.github.io/academy-bot/")
...
# =============================================================================
# لایه پایگاه داده (Data Access Layer)
# =============================================================================
async def init_db():
    """ایجاد جداول مورد نیاز پروژه در زمان راه‌اندازی"""
...
async def create_ticket(user_id: int, department: str) -> int:
    """ثبت تیکت جدید و برگرداندن شناسه آن"""
    async with aiosqlite.connect(DB_NAME) as db:
...
# =============================================================================
# ساخت کیبوردها به همراه دکمه مینی‌اپ
# =============================================================================
def get_menu_by_role(role: str) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()

    # دکمه ورود به محیط مینی‌اپ برای تمامی نقش‌ها
    builder.button(text="🚀 ورود به مینی‌اپ آکادمی", web_app=WebAppInfo(url=WEB_APP_URL))
    ...
    if role == "teacher":
        tch_builder = InlineKeyboardBuilder()
        tch_builder.button(text="📊 وضعیت و کارنامه فراگیران", callback_data="view_ranking")
        tch_builder.button(text="➕ طراحی آزمون / سوال", callback_data="create_card")
        tch_builder.button(text="📢 ارسال اطلاعیه همگانی", callback_data="teacher_broadcast")
        tch_builder.adjust(2, 1)
        builder.attach(tch_builder)

    return builder.as_markup()

# =============================================================================
# ورود و ثبت‌نام کاربر با لایسنس و نام رسمی
# =============================================================================
...
async def start_cmd(message: types.Message, state: FSMContext):
    ...
    if not user or user[1] == "guest":
        await message.answer(
            "سلام! به سامانه آکادمی خوش آمدید. 🌿\n\n"
            "برای شروع، لطفاً **کد لایسنس اختصاصی** خود را ارسال فرمایید:"
        )
    ...
    await message.answer(
        f"درود {name} عزیز، وقت بخیر! 👋\n"
        f"سطح دسترسی شما: **{role.upper()}**\n\n"
        f"{name} گرامی، به کدام بخش می‌خواهید دسترسی داشته باشید؟",
...
async def process_license(message: types.Message, state: FSMContext):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        if not lic:
            await message.answer("❌ لایسنس وارد شده معتبر نیست. لطفاً کد صحیح را بفرستید:")
            return

        if lic[1] == 1:
            await message.answer("❌ این لایسنس قبلاً استفاده شده و منقضی است.")
            return
    ...
    await message.answer(
        "✅ لایسنس با موفقیت تایید شد!\n\n"
        "جهت ثبت در پرونده و مکالمات، لطفاً **نام و نام خانوادگی** خود را تایپ و ارسال کنید:"
    )
...
async def process_full_name(message: types.Message, state: FSMContext):
    ...
    await state.clear()
    logger.info(f"کاربر {user_id} با نام {full_name} و نقش {role} ثبت شد.")

    welcome_text = (
        f"🌿 {full_name} عزیز، ثبت‌نام شما تکمیل شد!\n"
        f"سطح دسترسی شما: **{role.upper()}**\n\n"
        f"{full_name} گرامی، یکی از گزینه‌های زیر را انتخاب کنید:"
    )
    await message.answer(welcome_text, reply_markup=get_menu_by_role(role), parse_mode="Markdown")

# =============================================================================
# سیستم تیکتینگ و ارتباط فراگیر با پشتیبانی
# =============================================================================
...
async def select_ticket_dept(callback: types.CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="آموزشی و جلسات", callback_data="dept_آموزشی"),
            InlineKeyboardButton(text="فنی و فایل‌ها", callback_data="dept_فنی")
        ],
    ...
    await callback.message.edit_text(
        f"📩 {name} عزیز، به بخش پشتیبانی خوش آمدید.\nلطفاً دپارتمان مربوطه را انتخاب کنید:",
        reply_markup=kb,
        parse_mode="Markdown"
    )
...
async def prompt_ticket_msg(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        f"دپارتمان: **{dept}**\n\nلطفاً پیام، سوال یا فایل خود را ارسال فرمایید:",
        parse_mode="Markdown"
    )
...
async def ticket_submitted(message: types.Message, state: FSMContext):
    ...
    await message.answer(f"✅ {student_name} عزیز، تیکت شما با شناسه #{ticket_id} برای پشتیبانان ارسال شد.")
    await state.clear()
    ...
    try:
        ...
        logger.error(f"خطا در ارسال تیکت به گروه پشتیبانی: {err}")
...
async def start_admin_reply(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.reply(
        f"در حال پاسخ‌دهی به تیکت #{ticket_id}.\n"
        "پیام، صوت یا تصویر خود را ارسال فرمایید:"
    )
...
async def send_admin_reply(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.reply("✅ پاسخ با موفقیت به فراگیر تحویل داده شد.")
    except Exception as err:
        await message.reply(f"❌ خطا در ارسال پیام: {err}")
    await state.clear()
...
async def admin_close_ticket(callback: types.CallbackQuery):
    ...
    if ticket_data:
        try:
            await bot.send_message(chat_id=ticket_data[0], text=f"🔒 تیکت شماره #{ticket_id} شما مختومه اعلام شد.")
        except Exception:
            pass
    await callback.message.reply(f"تیکت #{ticket_id} با موفقیت بسته شد.")
    await callback.answer()

# =============================================================================
# مسیرهای آموزشی (ترم‌ها و جلسات)
# =============================================================================
...
async def show_courses(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text(
        f"📚 {name} عزیز، مسیر ترم‌های آموزشی:\nلطفاً ترم مورد نظر را انتخاب فرمایید:",
        reply_markup=kb,
        parse_mode="Markdown"
    )
    await callback.answer()
...
async def show_term_1(callback: types.CallbackQuery):
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="🎧 صوت جلسه ۱", callback_data="dl_audio_1"),
            InlineKeyboardButton(text="📄 جزوه PDF جلسه ۱", callback_data="dl_pdf_1")
        ],
        [InlineKeyboardButton(text="🔙 بازگشت", callback_data="courses")]
    ])
    await callback.message.edit_text("🟢 **ترم ۱:** فایل‌های آموزشی آماده دریافت هستند:", reply_markup=kb, parse_mode="Markdown")
    await callback.answer()
...
async def show_term_2(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text("🟡 **ترم ۲:** در حال برگزاری است و جلسات به مرور اضافه می‌شوند.", reply_markup=kb)
    await callback.answer()
...
async def send_pdf(callback: types.CallbackQuery):
    ...
    await callback.message.answer("📄 جزوه PDF جلسه ۱ آماده شد.")
...
async def send_audio(callback: types.CallbackQuery):
    ...
    await callback.message.answer("🎧 صوت آموزشی جلسه ۱ آماده شد.")

# =============================================================================
# موتور آزمون‌های فلش‌کارتی
# =============================================================================
...
async def start_quiz(callback: types.CallbackQuery):
    ...
    if not card:
        await callback.answer("هنوز فلش‌کارتی در سامانه ثبت نشده است.", show_alert=True)
        return
...
async def reveal_card(callback: types.CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="❌ بلد نبودم", callback_data="score_0"),
            InlineKeyboardButton(text="✅ بلد بودم (+۵)", callback_data="score_5")
        ],
        [InlineKeyboardButton(text="⏭ کارت بعدی", callback_data="quiz_flashcards")],
        [InlineKeyboardButton(text="🔙 اتمام", callback_data="back_main")]
    ])
    await callback.message.edit_text(f"❓ **پرسش:**\n{q}\n\n💡 **پاسخ:**\n{a}\n\nمیزان تسلط خود را انتخاب کنید:", reply_markup=kb, parse_mode="Markdown")
    await callback.answer()
...
async def add_score(callback: types.CallbackQuery):
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        await db.commit()
    await callback.answer("۵ امتیاز به شما اضافه شد! 🎉")
...
# =============================================================================
# طراحی آزمون و سوالات جدید (استاد و پشتیبان)
# =============================================================================
...
async def create_card_start(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(f"{user[2]} گرامی، صورت **پرسش** را بفرستید:")
...
async def create_card_q(message: types.Message, state: FSMContext):
    ...
    await message.answer("💡 حالا **پاسخ تشریحی** را ارسال فرمایید:")
...
async def create_card_a(message: types.Message, state: FSMContext):
    ...
    user = await get_user(message.from_user.id)
    await state.clear()
    await message.answer("✅ سوال با موفقیت ذخیره شد.", reply_markup=get_menu_by_role(user[1]))

# =============================================================================
# رتبه‌بندی، کارنامه و آمار کاربران
# =============================================================================
...
async def view_ranking(callback: types.CallbackQuery):
    ...
    text = "🏆 **جدول فعالیت فراگیران:**\n\n" if students else "هنوز کاربری ثبت نشده است."
...
async def supporter_stats(callback: types.CallbackQuery):
    ...
    text = f"📊 **آمار سامانه آکادمی:**\n\n🔹 اعضای فعال: **{total_active} نفر**\n🔹 کل فراگیران: **{total_students} نفر**"
    kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 بازگشت", callback_data="back_main")]])
    await callback.message.edit_text(text, reply_markup=kb, parse_mode="Markdown")
    await callback.answer()
...
async def attendance_prompt(callback: types.CallbackQuery, state: FSMContext):
    await callback.message.answer("لطفاً شناسه عددی (User ID) فراگیر را بفرستید:")
...
async def process_attendance_id(message: types.Message, state: FSMContext):
    if not message.text.isdigit():
        await message.answer("شناسه باید فقط عدد باشد. مجدداً ارسال کنید:")
        return
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="✅ حاضر", callback_data=f"att_present_{student_id}"),
            InlineKeyboardButton(text="❌ غایب", callback_data=f"att_absent_{student_id}")
        ],
        [InlineKeyboardButton(text="🔙 انصراف", callback_data="back_main")]
    ])
    await state.clear()
    await message.answer(f"وضعیت حضور برای **{student_title}**:", reply_markup=kb, parse_mode="Markdown")
...
# =============================================================================
# اطلاعیه همگانی استاد
# =============================================================================
@dp.callback_query(F.data == "teacher_broadcast")
async def start_broadcast(callback: types.CallbackQuery, state: FSMContext):
    user = await get_user(callback.from_user.id)
    if not user or user[1] != "teacher":
        await callback.answer("دسترسی فقط مخصوص استاد است.", show_alert=True)
        return

    await callback.message.answer(f"{user[2]} عزیز، متن اطلاعیه را بفرستید:")
